		                ...
		                throws IOException 
	    {
			...
			if (len < n50contigthreshold)
			{
				if (len >= 50)
				{
					...
					medcov += cov * len;
					reporter.incrCounter("Contrail", "mednodes", 1);
				}
				...
				reporter.incrCounter("Contrail", "smallnodes", 1);
			}
			else
			{
...
//				my ($len, $fdegree, $rdegree, $cov) = split /\t/, $_;
	...
	{
		String inputPath  = "/users/mschatz/try/04-notipscmp"; //args[0];
		String outputPath = "/users/mschatz/try/04-notipscmp.stats"; //args[1];
		ContrailConfig.N50_TARGET = 1234;
